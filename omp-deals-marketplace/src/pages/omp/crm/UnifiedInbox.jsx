...
export const UnifiedInbox = () => {
  ...
  const [mobileView, setMobileView] = useState('list'); // 'list' | 'chat' | 'vehicle'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '0.75rem' : '1.25rem', height: isMobile ? 'calc(100vh - 100px)' : 'calc(100vh - 120px)' }}>
      {/* Header (Always visible on desktop, or on mobile when viewing the leads list) */}
      {(!isMobile || mobileView === 'list') && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem', flexShrink: 0 }}>
          <div>
            ...
            <h1 style={{ fontSize: isMobile ? '1.25rem' : '1.5rem', fontWeight: 800, color: 'var(--text-primary)', margin: '0.2rem 0 0 0' }}>
              OMP Deals Unified Omnichannel Inbox
            </h1>
          </div>

          {/* Channel Filters */}
          <div style={{ display: 'flex', gap: '0.4rem', backgroundColor: 'var(--surface)', padding: '0.25rem', borderRadius: '8px', border: '1px solid var(--border)', overflowX: 'auto', maxWidth: '100%' }}>
      ...
      {/* Main Inbox 3-Column Split (Desktop) or 1-Panel Master-Detail (Mobile) */}
      <div
        style={{
          flex: 1,
          display: isMobile ? 'flex' : 'grid',
          flexDirection: isMobile ? 'column' : undefined,
          gridTemplateColumns: isMobile ? undefined : '320px 1fr 280px',
      ...
      >
        ...
        {(!isMobile || mobileView === 'list') && (
          <div style={{ flex: 1, width: isMobile ? '100%' : undefined, backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
        ...
        {(!isMobile || mobileView === 'chat') && (
          <div style={{ flex: 1, width: isMobile ? '100%' : undefined, backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            {/* Thread Header */}
            <div style={{ padding: isMobile ? '0.75rem' : '0.85rem 1.25rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'var(--surface-secondary)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '0.92rem' }}>{activeThread.customerName}</div>
                  <div style={{ fontSize: '0.72rem', color: '#0284c7', fontWeight: 600 }}>
                    {activeThread.channel} • {activeThread.vehicle.year} {activeThread.vehicle.model}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                {isMobile && (
                  <button
                    ...
                    title="View Vehicle Info & Desk Deal"
                  >
                    <Car size={13} />
                    <span>{activeThread.vehicle.price}</span>
                  </button>
                )}
                <button style={{ backgroundColor: 'rgba(16, 185, 129, 0.15)', color: '#10b981', border: '1px solid rgba(16, 185, 129, 0.3)', padding: '0.35rem 0.65rem', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.3rem', cursor: 'pointer' }}>
                  <Phone size={13} /> {!isMobile && 'Click-to-Call'}
                </button>
              </div>
            </div>
            ...
            {/* Quick AI Templates */}
            <div style={{ padding: '0.5rem 0.75rem', borderTop: '1px solid var(--border)', display: 'flex', gap: '0.4rem', overflowX: 'auto', backgroundColor: 'var(--surface-secondary)' }}>
              <span style={{ fontSize: '0.68rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '0.2rem', flexShrink: 0 }}>
                <Sparkles size={12} color="#0284c7" /> AI:
              </span>
              {[
            ...
            {/* Message Input Bar */}
            <form onSubmit={handleSendMessage} style={{ padding: isMobile ? '0.5rem' : '0.75rem 1rem', borderTop: '1px solid var(--border)', display: 'flex', gap: '0.5rem', backgroundColor: 'var(--surface)' }}>
              <input
                type="text"
                value={messageText}
                onChange={(e) => setMessageText(e.target.value)}
                placeholder={`Type reply to ${activeThread.customerName}...`}
        ...
        {(!isMobile || mobileView === 'vehicle') && (
          <div style={{ flex: isMobile ? 1 : undefined, width: isMobile ? '100%' : undefined, backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem', overflowY: 'auto', boxShadow: 'var(--shadow-sm)' }}>
            {isMobile && (
              <button
